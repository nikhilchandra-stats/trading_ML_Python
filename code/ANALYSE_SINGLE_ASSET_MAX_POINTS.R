...
query_var <-
  glue::glue("SELECT * FROM single_asset_advanced_optimisation
             WHERE Asset = '{asset}'")
asset_data_optim <-
  DBI::dbGetQuery(conn = db_optim_data_con,
                  statement = query_var)
DBI::dbDisconnect(db_optim_data_con)

return_col <- "long_return"

summary_of_return_dist <-
  asset_data_optim %>%
  ungroup() %>%
  group_by(Asset, trade_col, profit_factor_long, stop_factor_long,
           period_var_long) %>%
  summarise(
    min_return = min(!!as.name(return_col) ,na.rm = T),
    low_25_return = quantile(!!as.name(return_col), 0.25 ,na.rm = T),
    mean_return = mean(!!as.name(return_col), na.rm = T),
    high_75_return = quantile(!!as.name(return_col), 0.75 ,na.rm = T),
    high_90_return = quantile(!!as.name(return_col), 0.90 ,na.rm = T),
    high_95_return = quantile(!!as.name(return_col), 0.95 ,na.rm = T),
    max_return = max(!!as.name(return_col) ,na.rm = T)
  )
  ...
  function(
    ...
    mutate(
      min_point_in_trade_long =
        select(., contains("Long_period_return_")) %>%  reduce(pmin),
      max_point_in_trade_long =
        select(., contains("Long_period_return_")) %>% reduce(pmax) ,

      min_point_in_trade_short =
        select(., contains("Short_period_return_")) %>%  reduce(pmin),
      max_point_in_trade_short =
        select(., contains("Short_period_return_")) %>%  reduce(pmax),

      max_point_period_long =
        case_when(
          Long_period_return_30_Price == max_point_in_trade_long ~ 30,
          Long_period_return_29_Price == max_point_in_trade_long ~ 29,
          Long_period_return_28_Price == max_point_in_trade_long ~ 28,
          Long_period_return_27_Price == max_point_in_trade_long ~ 27,
          Long_period_return_26_Price == max_point_in_trade_long ~ 26,
          Long_period_return_25_Price == max_point_in_trade_long ~ 25,
          Long_period_return_24_Price == max_point_in_trade_long ~ 24,
          Long_period_return_23_Price == max_point_in_trade_long ~ 23,
          Long_period_return_22_Price == max_point_in_trade_long ~ 22,
          Long_period_return_21_Price == max_point_in_trade_long ~ 21,
          Long_period_return_20_Price == max_point_in_trade_long ~ 20,
          Long_period_return_19_Price == max_point_in_trade_long ~ 19,
          Long_period_return_18_Price == max_point_in_trade_long ~ 18,
          Long_period_return_17_Price == max_point_in_trade_long ~ 17,
          Long_period_return_16_Price == max_point_in_trade_long ~ 16,
          Long_period_return_15_Price == max_point_in_trade_long ~ 15,
          Long_period_return_14_Price == max_point_in_trade_long ~ 14,
          Long_period_return_13_Price == max_point_in_trade_long ~ 13,
          Long_period_return_12_Price == max_point_in_trade_long ~ 12,
          Long_period_return_11_Price == max_point_in_trade_long ~ 11,
          Long_period_return_10_Price == max_point_in_trade_long ~ 10,
          Long_period_return_9_Price == max_point_in_trade_long ~ 9,
          Long_period_return_8_Price == max_point_in_trade_long ~ 8,
          Long_period_return_7_Price == max_point_in_trade_long ~ 7,
          Long_period_return_6_Price == max_point_in_trade_long ~ 6,
          Long_period_return_5_Price == max_point_in_trade_long ~ 5,
          Long_period_return_4_Price == max_point_in_trade_long ~ 4,
          Long_period_return_3_Price == max_point_in_trade_long ~ 3
        )
    ...
    mutate(
      ...
      summarise(
        ...
        min_point_in_trade_long = mean(min_point_in_trade_long, na.rm=T),
        max_point_in_trade_long = mean(max_point_in_trade_long, na.rm=T),
        min_point_in_trade_short = mean(min_point_in_trade_short, na.rm=T),
        max_point_in_trade_short = mean(max_point_in_trade_short, na.rm=T),
        max_point_period_long = mean(max_point_period_long, na.rm = T)
      ) %>%
      ungroup()
...
for (i in 1:dim(best_trade_setups)[1] ) {

  asset_of_interest <- best_trade_setups$Asset[i]
  profit_factor_long <- best_trade_setups$profit_factor_long[i]
  profit_factor_long_fast <- best_trade_setups$profit_factor_long_fast[i]
  profit_factor_long_fastest <- best_trade_setups$profit_factor_long_fastest[i]

  stop_factor_long <- best_trade_setups$stop_factor_long[i]
  profit_factor_short <- best_trade_setups$profit_factor_short[i]
  stop_factor_short <- best_trade_setups$stop_factor_short[i]
  ...
  if(i == 1) {
    write_table_sql_lite(conn = model_optimisation_store_db,
                         .data = temp,
                         table_name = "single_asset_max_point_optimisation",
                         overwrite_true = TRUE)
  } else {

    append_table_sql_lite(conn = model_optimisation_store_db,
                          .data = temp,
                          table_name = "single_asset_max_point_optimisation")
...
get_best_trade_end_point <-
  function(
    ...
    temp_summary <-
      all_max_point_sims %>%
      # filter(max_point_in_trade_long > 0) %>%
...
#---------------------Restimate All Parameters with highest points
#------------------------------------------------------------------
param_tibble <-
  c(18,15,12) %>%
  # c(18) %>%
...
for (k in 1:length(asset_list) ) {
  ...
  results_summary <-
    ...
    mutate(long_only_cum = cumsum(long_return),
           ...
           win_or_loss = ifelse(Total_Return > 3.5, 1, 0) ) %>%
    group_by(
      stop_factor_short , profit_factor_short ,
      period_var_short , profit_factor_long , stop_factor_long,
      period_var_long, profit_factor_long_fast, profit_factor_long_fastest,
    ...
    summarise(
      ...
      wins_or_loss_3_dollar_min = sum(win_or_loss),
