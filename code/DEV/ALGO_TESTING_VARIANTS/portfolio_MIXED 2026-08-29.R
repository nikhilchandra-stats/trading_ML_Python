...
model_predicted_data <-
  ...
  mutate(
    ...
    pnorm_2000_roll_2000 = slider::slide_dbl(.x  = pnorm_1001, .f = ~ mean(.x, na.rm = T), .before = 2000),
...
names(model_predicted_data_raw)
trade_statment <-
  "
(predicted_portfolio > 0 & predicted_portfolio < 15)
...
return_thresh <- 5
auc_roc_list <- list()
col_to_test <- "pnorm_50"
col_to_test_port <- "pnorm_50"
c = 0
...
auc_roc_list <- list()
return_thresh <- 5
col_to_test <- "pred_10000_mean_roll_1500"
col_to_test_mean <- "pred_10000_mean_roll_2000"
col_to_test_sd <- "pred_10000_sd_roll_2000"

col_to_test_port <- "pred_10000_mean_roll_1500"
col_to_test_mean_port <- "pred_10000_mean_roll_2000"
col_to_test_sd_port <- "pred_10000_sd_roll_2000"
c = 0
...
auc_roc_filt <-
  ...
  filter(threshold >= 0) %>%
  filter(Total_Trades > 4000) %>%
  filter(!is.na(TRUE_pos_rate), Final_Return > 1000) %>%
  group_by(Asset) %>%
