...
get_res_sup_slow_fast_fractal_data <-
  function(
    ...
    prob_tibble_XX <- get_cauchy_params_by_asset(
    ...
    testing_data <-
      ...
      group_by(Asset) %>%
      mutate(
        running_mean_prob_diff = slider::slide_dbl(prob_diff, .f = ~ mean(.x, na.rm = T), .before = 200),
        running_sd_prob_diff = slider::slide_dbl(prob_diff, .f = ~ sd(.x, na.rm = T), .before = 200)
      ) %>%
      ungroup()
