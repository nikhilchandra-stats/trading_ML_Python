  ...
  function(
    ...
    for (i in 1:length(thresh_vector)) {
      ...
      required_sample_length <-
        round(length(Trade_Data$win_loss)/10)

      if(required_sample_length >= 50) {

        for (j in 1:3000) {
          random_testing_perc[j] <-
            sum(Trade_Data$win_loss %>% sample(size = 480), na.rm = T)/480
          random_testing_return[j] <-
            sum(Trade_Data %>% pull(!!as.name(period_return_col)) %>% sample(size = 480), na.rm = T)
        }
