...
trade_tracker_DB_path <-
  "D:/trade_data/trade_tracker_daily_buy_close 2.db"
...
while (run_all == 1) {
  ...
  if(current_minute >= 0 &
    ...
    if(u1 != 'error' & u2 != 'error' & u3 != 'error' & u4 != 'error'){
      ...
      asset_optimisation_store_path =
        "D:/trade_data/single_asset_improved_asset_optimisation_more_cop 2.db"
      ...
      best_trade_setups <-
        get_best_trade_setup_sa_old_laptop(
          model_optimisation_store_path =
            "D:/trade_data/single_asset_advanced_optimisation Algo Summary 2.db",
    ...
    if(u1 == 'error' | u2 == 'error' | u3 == 'error' | u4 != 'error') {
      Sys.sleep(60*5)
      upload_increment <- upload_increment + 2
    }
    ...
    rm(cleaned_trade_details, http_return, total_trades, single_asset_model_trades)
    gc()
  ...
  if(trades_closed == 0 &
    ...
    trades_from_DB <-
      ...
      filter(Asset %in% Asset_Available)
