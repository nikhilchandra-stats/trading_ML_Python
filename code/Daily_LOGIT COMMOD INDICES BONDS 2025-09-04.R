...
stop_value_var = 0.5
profit_value_var = 1
available_assets <- asset_data_daily_raw_ask %>% filter(str_detect(Asset, "XAG|XAU|SPX500|USB05Y_USD|USB10Y_USD|CH20_CHF|UK100_GBP|JP225Y_JPY|EU50_EUR|FR40_EUR|JP225_USD|AU200_AUD|ESPIX_EUR|SG30_SGD|DE10YB_EUR|WTICO_USD|BCO_USD|NATGAS_USD|XCU_USD")) %>% pull(Asset) %>% unique()
...
NN_sims_db <- "C:/Users/Nikhil Chandra/Documents/trade_data/COMMOD_INDICES_BONDS.db"
  ...
  for (k in 175:length(date_sequence)) {
    ...
    for (i in 1:length(available_assets)) {
      ...
      check_completion <- safely_generate_NN(
        ...
        NN_path = "C:/Users/Nikhil Chandra/Documents/trade_data/asset_specific_NN_Daily_Quant_3/",
    ...
    if(redo_db == TRUE) {
      write_table_sql_lite(.data = all_asset_logit_results,
                           table_name = "COMMOD_INDICES_BONDS",
                           conn = NN_sims_db_con)
      redo_db = FALSE
    } else {
...
all_results_ts_dfr <- DBI::dbGetQuery(conn = NN_sims_db_con,
                                      statement = "SELECT * FROM COMMOD_INDICES_BONDS")
