...
NN_sims_db <- "C:/Users/nikhi/Documents/trade_data/BONDS_METALS_INDX_1_sims_short.db"
...
for (j in 1:dim(params_to_test)[1]) {
  ...
  for (k in 237:length(date_sequence)) {
...
all_asset_logit_results_sum <-
  ...
  filter(simulations >= 50, edge > 0, outperformance_count > 0.5, risk_weighted_return_mid > 0.1) %>%
  group_by(Asset) %>%
  slice_max(Trades)
