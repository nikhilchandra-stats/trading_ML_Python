    ...
    if(u1 != "error" & u2 != "error" & u3 != "error" & u4 != "error") {
      ...
      if(
        ...
        MIXED_TRADE_STATEMENT <-
          trade_statement_DFR %>%
          filter(str_detect(algo_name, "USD_ONLY_EXTND_risk10_stop10_prof20_end50_SIG1")) %>%
...
# Close Trades ------------------------------------------------------------
  ...
  if(trades_closed == 0 &
     now(tzone = "Australia/Canberra") >= as_datetime("2026-08-31 07:00:00", tz = "Australia/Canberra")) {
