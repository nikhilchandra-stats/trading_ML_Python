...
NN_sims_db <- "C:/Users/nikhi/Documents/trade_data/INDICES_15M_NN_sims_logit_Short.db"
...
all_asset_logit_results_sum <-
  ...
  filter(edge > 0, Outperformance_Perc > 0.55, total_sims >= 100, risk_weighted_return_mid >= 0.1) %>%
  group_by(Asset) %>%
  slice_max(risk_weighted_return_mid, n = 1) %>%
  group_by(Asset) %>%
  slice_max(Outperformance_Perc) %>%
  group_by(Asset) %>%
  slice_max(Trades)
