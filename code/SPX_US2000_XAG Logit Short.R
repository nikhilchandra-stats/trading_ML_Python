...
SPX_XAG_US2000_Short_Data_5_10 <-
  get_trade_results_ts_aud(
    tagged_trades = SPX_XAG_US2000_Short_trades,
    stop_factor = 5,
    profit_factor = 10,
...
SPX_XAG_US2000_Short_Data_8_12 <-
  get_trade_results_ts_aud(
    tagged_trades = SPX_XAG_US2000_Short_trades,
    stop_factor = 8,
    profit_factor = 12,
...
all_short_trades <-
  SPX_XAG_US2000_Short_Data %>%
  bind_rows(SPX_XAG_US2000_Short_Data_5_10)%>%
  bind_rows(SPX_XAG_US2000_Short_Data_8_12)

trade_actual_db <- "C:/Users/nikhi/Documents/trade_data/trade_actuals_short.db"
...
rm(all_short_trades, SPX_XAG_US2000_Short_Data, SPX_XAG_US2000_Short_Data_8_12)
gc()
#-------------------------------------------------------------
trade_actual_db <- "C:/Users/nikhi/Documents/trade_data/trade_actuals_short.db"
trade_actual_db_con <- connect_db(trade_actual_db)
...
date_sequence <- seq(as_date("2018-01-01"), as_date("2025-06-01"), "week") %>%
  # sample(200) %>%
  keep(
    ~ as_date(.x) >= (as_date("2018-01-01") + lubridate::dminutes(15*40000))
  )
all_results_ts <- list()
profit_value_var <- 14
stop_value_var <- 9

NN_sims_db <- "C:/Users/nikhi/Documents/trade_data/INDICES_15M_NN_sims_short.db"
...
params_to_test <- tibble(
  ...
  trade_direction_var = c("Short", "Short", "Short", "Short")
)
redo_db = TRUE
  ...
  for (k in 1:length(date_sequence)) {
    ...
    for (i in 1:length(available_assets)) {
      ...
      check_completion <-
        ...
        NN_path = "C:/Users/nikhi/Documents/trade_data/asset_specific_NNs_short/",
...
all_asset_logit_results_sum <-
  ...
  filter(edge > 0, Outperformance_Perc > 0.5, total_sims >= 30, risk_weighted_return_mid >= 0.14) %>%
  group_by(Asset) %>%
