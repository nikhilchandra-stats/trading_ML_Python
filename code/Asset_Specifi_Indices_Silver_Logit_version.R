...
params_to_test <-
  tibble(
    ...
    p_value_thresh_for_inputs = c(0.00001, 0.15, 0.2, 0.3, 0.4),
...
for (j in 1:dim(params_to_test)[1]) {
  ...
  for (k in 278:length(date_sequence)) {
...
all_asset_logit_results_sum <-
  ...
  filter(simulations >= 100, edge > 0.16, outperformance_perc > 0.52, risk_weighted_return_mid > 0.05) %>%
  group_by(Asset) %>%
